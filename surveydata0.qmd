...
2.  **Effective Sample Size:** The DEFF allows you to calculate the "effective sample size" ($n_{eff} = n / DEFF$) **for a particular estimate**, which is the size of an SRS that would yield the same level of precision for that estimate. Because the DEFF (and hence $n_{eff}$) is estimate-specific, it varies from one statistic to another rather than being a single property of the dataset. This is a powerful tool for communicating the statistical power of a study.
...
-   **Predictive Performance:** Measures like pseudo-R² should be calculated using survey-weighted versions. For assessing a model's ability to discriminate between outcomes, the Area Under the ROC Curve (AUC) should be calculated as a **weighted AUC** to ensure the measure is representative of the model's performance in the target population. Note that a weighted-only AUC or goodness-of-fit statistic (built with `id = ~1`) accounts for the **weights** but not for stratification and clustering; a fully design-correct version additionally incorporates the strata and PSU/cluster identifiers (or replicate weights).
...
(b) **Reporting of lonely/singleton PSU**:
...
(c) **Reporting of reliability of estimates**: 
...
- Relative standard error (RSE) or %RSE = (Standard error of estimate / Estimate) * 100: Should be <30% for "stable" estimates per [CDC guidelines](https://wwwn.cdc.gov/nchs/nhanes/tutorials/reliabilityofestimates.aspx); suppress or flag unstable ones (e.g., wide CIs).
...
For reporting sex- and gender-based analyses (not necessarily part of survey data analysis), we recommend [SAGER guidelines](https://doi.org/10.1186/s41073-016-0007-6) [@heidari2016sex]. We apply SAGER checklist [@van2022sex] [[link](https://doi.org/10.3897/ese.2022.e86910)] on an example article [@karim2025examining] [[link](https://doi.org/10.1016/j.focus.2024.100282)].
