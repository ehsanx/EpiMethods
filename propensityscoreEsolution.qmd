...
-   Step 3: Balance checking using SMD. Consider SMD \<0.2 as a good covariate balancing.
...
tab1m <- CreateTableOne(vars = vars, strata = "nocturia", data = matched.data, 
                           test = F, includeNA = F)
print(tab1m, smd = TRUE, format = "p") # All SMDs <0.2
```
...
-   Step 3: Balance checking using SMD. Consider SMD \<0.2 as a good covariate balancing. Remember, you need to consider matching weights in checking the covariate balance.
...
match.obj3 <- matchit(ps.formula3, 
                      ...
                      caliper = caliper3, 
                      ratio = 4)
dat.analytic$PS3 <- match.obj3$distance
#summary(match.obj3$distance)
...
tab13m <- svyCreateTableOne(vars = vars, strata = "nocturia", data = w.design.m, 
                           test = F, includeNA = F)
print(tab13m, smd = TRUE, format = "p") # All SMDs <0.2
```
