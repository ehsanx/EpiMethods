...
### [Mediation Software](mediation4.html)

This tutorial walks through two complementary approaches to causal mediation: a manual inverse probability weighting (IPW) method that follows the step-by-step construction in the [slides](mediation0.html), and a parametric approach using the `regmedint` package. After creating a complete-case dataset with binary exposure, mediator and outcome, we build counterfactual copies of each observation, derive mediator weights from a logistic regression, and fit a weighted outcome model to obtain the natural direct effect, natural indirect effect, total effect and proportion mediated. We then fit an equivalent mediation model using `regmedint`, which computes the same estimands via parametric standardization and delta–method standard errors. Both approaches yield broadly consistent conclusions, with similar total and mediated effects, while small differences arise from the different estimating strategies and the evaluation of `regmedint` effects at the mean covariate profile. For clarity, all analyses here intentionally ignore the complex survey design to focus on illustrating mediation concepts.
