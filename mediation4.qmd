...
::: callout-warning
**Caveat on `as.numeric()` applied to a factor.** Calling `as.numeric()` on a factor returns its **integer level codes** (1, 2, 3, ...), not a meaningful numeric measurement. For a multi-level categorical covariate such as `race` or `bmi` category, this forces `regmedint` to treat it as a single continuous term, and `c_cond` is then set at a fractional "mean code" (for example `race` ≈ 1.88, `bmi` ≈ 2.49) that does not correspond to any real category. This is a structurally different covariate model from the dummy-expanded factors used in the manual IPW fit, and it is one reason the two approaches do not match exactly. A more faithful implementation would keep genuinely continuous/binary covariates as numeric and expand multi-level factors into dummy indicators (setting the corresponding `c_cond` entries to the observed proportions).
:::
...
- Effects are evaluated at the mean covariate profile `c_cond_vec`, not over the full covariate distribution; this, together with the factor-coding difference noted in Section 7.1, makes the comparison with the IPW based approach approximate rather than exact.
...
- **The two approaches fit structurally different covariate models** (the leading reason for the gap here). The manual IPW fit uses the factor covariates with their dummy-expanded levels, whereas the `regmedint` fit recodes each factor to integer level codes via `as.numeric()` and treats it as a single linear term evaluated at a fractional "mean code" (see the caveat in Section 7.1). These are not the same adjustment set, so the estimands being compared are not identical and the agreement should be read as approximate rather than "close enough."
