...
The Root Mean Square Error (RMSE) is calculated to measure the average magnitude of the errors between predicted and observed values. The RMSE divides the SSE by the sample size `n` (matching `caret::RMSE`). The closely related quantity `sqrt(SSE/(n-p))`, which divides by the residual degrees of freedom, is the residual standard error reported by `summary(lm)` rather than the RMSE.
