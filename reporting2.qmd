...
[LaTeX](https://en.wikipedia.org/wiki/LaTeX) is a high-quality software system for typesetting document. LaTeX is designed for the production of technical and scientific documentation, such as book, articles. 

Get an account on [Overleaf](https://www.overleaf.com/). (Overleaf merged with the now-discontinued ShareLaTeX in 2017, so older "ShareLaTeX" links now redirect to Overleaf.)

### Table conversion

From MS word to latex / markdown / HTML in [TablesGenerator](http://www.tablesgenerator.com/).

You can use `tableone` package to generate csv file, and then import them in the TablesGenerator to convert to HTML to paste to doc file! In the snippet below, `tab1` is a `tableone` object (e.g., created earlier with `CreateTableOne()`); it is assumed to be defined elsewhere in your analysis.
...
[draw.io](https://www.draw.io/) (also available at [app.diagrams.net](https://app.diagrams.net/))
...
[Google Docs](https://docs.google.com/) offers a platform for real-time collaborative writing. Multiple users can edit documents simultaneously, and changes are saved automatically. Note that [Google Docs](https://docs.google.com/) requires sign-in for a Google account.   
...
| Document Creation & Editing  | [Overleaf](https://www.overleaf.com/)                                                                         | Collaborative LaTeX editor online.                                          |
