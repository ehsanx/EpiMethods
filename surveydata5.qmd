...
::: callout-warning
`svyROCw` (and `AL.gof` below) incorporate only the survey **weights** (`id = ~1`); they do **not** account for stratification or clustering. The resulting weighted AUC and Archer-Lemeshow test are therefore design-aware only with respect to weighting. For a fully design-correct analysis that also uses strata and PSU/cluster information, use the `AL.gof2` function shown later in this tutorial, or see the [updated NHANES tutorial](surveydata9c.html).
:::
