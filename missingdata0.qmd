...
### Checklist for Reporting Missing Data Analysis Using Multiple Imputation

| **Item** | **Description** |
|-----------|-----------------|
| **1. Justify Mechanism** | State the assumed missing-data mechanism (typically *Missing at Random*, MAR). Provide a subject-matter rationale for why MAR is plausible in the NHANES context (e.g., missingness related to health status, age, or survey design features). |
| **2. Report Missingness Extent** | For each variable, report the number and percentage of missing values. Highlight variables with high missingness (e.g., >20–30%) and discuss whether they were imputed, handled separately, or excluded. Include an overall summary (e.g., proportion of participants with ≥1 missing variable). |
| **3. Describe Data Exclusions** | Document any records excluded from the analytic dataset (e.g., missing weights or design variables). Report the resulting sample size and, if appropriate, use a flow diagram to illustrate inclusion/exclusion and missingness. |
| **4. Conduct MCAR Diagnostics** | Report results from an exploratory test for *Missing Completely at Random* (e.g., Little’s MCAR test), acknowledging the test’s limitations (low power and restrictive assumptions). |
| **5. Detail Imputation Model** | List all variables included in the imputation model—outcome(s), exposures, confounders, auxiliary variables, and survey design variables (weights, strata, clusters). Describe handling of structural or skip-pattern missingness. |
| **6. Specify Imputation Parameters** | Report the number of imputations (*m*) and justify it (e.g., one per percent of incomplete cases). Indicate the number of iterations used for convergence and the imputation method for each variable type (e.g., predictive mean matching for continuous, logistic for binary). |
| **7. Report Software & Seed** | State the software, version, and key packages used (e.g., R 4.4.0, *mice* 3.16.0). Document the random seed(s) or initialization method to ensure reproducibility. |
| **8. Describe Analysis & Pooling** | Explain how survey design features (weights, strata, clusters) were incorporated into the analysis. Describe how estimates were pooled across imputations (e.g., Rubin’s Rules) and report the fraction of missing information (λ) for key parameters when available. |
| **9. Provide Diagnostics** | Present convergence diagnostics (e.g., trace plots) and distributional checks (e.g., density plots, mean comparisons) to demonstrate imputation stability and plausibility of imputed values. |
| **10. Include Sensitivity Analyses** | Summarize results from sensitivity analyses assessing robustness to different assumptions (e.g., compare MI with complete-case results, re-run with new seeds, or apply delta-adjustments for MNAR scenarios). |
| **11. Discuss Limitations** | In the discussion, reflect on the potential impact of missing data on study conclusions. Revisit the plausibility of MAR, acknowledge unmeasured factors, and discuss possible bias if data are MNAR. |


### Assessment of Missing Data Analysis Reporting

We take *Hossain et al. (2022)* [@hossain2022association] as an example to assess adherence to the above checklist.

| **Item** | **Description** | **Manuscript Compliance: Met vs. Not Met** |
|-----------|-----------------|--------------------------------------------|
| **1. Justify Mechanism** | State the assumed missing-data mechanism (typically *Missing at Random*, MAR). Provide a subject-matter rationale for why MAR is plausible in the NHANES context. | **Not Met.** The manuscript does not state an assumed missing-data mechanism (e.g., MCAR, MAR) for either the primary complete-case analysis or the multiple imputation sensitivity analysis. <br><br> *What they should have written:* “For our primary complete-case analysis, we assumed the data were Missing Completely at Random (MCAR), though we acknowledge this is a strong assumption. For our sensitivity analysis, we assumed data were Missing at Random (MAR), which is plausible as missingness on covariates like income is likely related to observed demographic variables also collected in the NHANES survey.” |
| **2. Report Missingness Extent** | For each variable, report the number and percentage of missing values. Highlight variables with high missingness and discuss whether they were imputed, handled separately, or excluded. | **Partially Met.** The manuscript provides overall numbers of excluded participants but does not detail the extent of missingness for each covariate. <br><br> *What they wrote:* “The flowchart in Figure 1 shows that from a sample of 48,252, there were ‘Missing values in the outcome/exposure (N=5,068)’ and ‘Missing values in covariates (N=8,122),’ leading to the complete-case sample of 35,062.” <br><br> *What they should have written:* A supplementary table listing each covariate (e.g., education, income, smoking) and its missingness should have been included. For example: “Missing data was highest for annual family income (12.5%) and physical activity (8.2%).” |
| **3. Describe Data Exclusions** | Document any records excluded from the analytic dataset (e.g., missing weights or design variables). Report the resulting sample size and, if appropriate, use a flow diagram. | **Met.** The manuscript uses a flow diagram (Figure 1) to clearly document exclusions. <br><br> *What they wrote:* “We excluded participants with arthritis other than RA, including 5233 with osteoarthritis, 90 with psoriatic arthritis, and 1506 with other arthritis (Fig. 1).” The flowchart also shows the exclusion of 46,235 participants aged <20 years and 5,068 with invalid/missing outcome or exposure information. |
| **4. Conduct MCAR Diagnostics** | Report results from an exploratory test for *Missing Completely at Random* (e.g., Little’s test), acknowledging its limitations. | **Not Met.** The manuscript does not mention performing or reporting any formal diagnostic tests for the MCAR assumption, which is required for unbiased complete-case analysis. <br><br> *What they should have written:* “We conducted Little’s MCAR test, which was statistically significant (p < .001), suggesting that the MCAR assumption is likely violated and supporting the need for multiple imputation under the MAR assumption.” |
| **5. Detail Imputation Model** | List all variables included in the imputation model—outcome(s), exposures, confounders, auxiliary variables, and survey design variables (weights, strata, clusters). | **Not Met.** The manuscript does not list variables used in the imputation model in the main text. It refers to an appendix that was not available for review. <br><br> *What they should have written:* “Our imputation model included the outcome (CVD), exposure (RA), and all covariates in Table 1. To improve MAR plausibility, we also included auxiliary variables (e.g., age, BMI, smoking) and survey design variables (strata, cluster, and weights) as predictors.” |
| **6. Specify Imputation Parameters** | Report the number of imputations (*m*) and justify it. Indicate iterations for convergence and imputation methods for each variable type. | **Not Met.** The manuscript does not report the number of imputations or algorithm details. <br><br> *What they should have written:* “We generated *m = 40* imputations (slightly above the maximum percentage of missingness for any variable). The chained equations algorithm ran for 20 iterations to ensure convergence. Predictive mean matching was used for continuous variables, logistic regression for binary variables.” |
| **7. Report Software & Seed** | State the software, version, and key packages used. Document random seed(s) for reproducibility. | **Partially Met.** Software and packages are listed, but versions and random seed are not reported. <br><br> *What they wrote:* “We used R 4.0.5; the *survey* package for design-adjusted models, *MatchIt* for propensity score analysis, and *mice* for multiple imputation.” <br><br> *What they should have written:* “We used R 4.0.5 with the *mice* package (v3.13.0). A random seed of 123 ensured reproducibility.” |
| **8. Describe Analysis & Pooling** | Explain how survey design features were incorporated into the analysis and how estimates were pooled (e.g., Rubin’s Rules). | **Partially Met.** The manuscript states survey adjustment but omits pooling method details. <br><br> *What they wrote:* “All analyses were adjusted for survey weights, clusters, and strata to account for complex survey design.” <br><br> *What they should have written:* “Analyses for each imputed dataset were combined using Rubin’s Rules to account for both sampling and imputation uncertainty.” |
| **9. Provide Diagnostics** | Present convergence diagnostics (e.g., trace plots) and distributional checks to show imputation stability and plausibility. | **Not Met.** No diagnostics for the MI procedure are presented. <br><br> *What they should have written:* “Convergence was confirmed via trace plots showing stable means and SDs of imputed variables after 10 iterations. Density plots comparing imputed and observed distributions indicated good overlap.” |
| **10. Include Sensitivity Analyses** | Summarize results from sensitivity analyses assessing robustness to different assumptions (e.g., MI vs. complete-case). | **Met.** The manuscript describes conducting and reporting MI-based sensitivity analyses. <br><br> *What they wrote:* “We conducted a sensitivity analysis using multiple imputation… The RA–CVD association estimate did not change substantially (eTable 4).” |
| **11. Discuss Limitations** | Reflect on the potential impact of missing data on conclusions. Revisit MAR plausibility and discuss possible MNAR bias. | **Met.** The limitations section appropriately discusses potential bias from missing data and assumptions. <br><br> *What they wrote:* “The results could be biased due to exclusions of those without RA or CVD data… Sensitivity analyses using MI and propensity score matching produced similar results, indicating robustness to missingness assumptions.” |
