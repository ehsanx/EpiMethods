...
-   You may compare the outputs with the regular linear model. **lm** ignores the within-subject correlation, so its SEs can be misleading. The direction depends on the type of covariate: for between-subject covariates with a positive intra-class correlation, **lm** tends to **underestimate** the SE (anti-conservative), whereas for within-subject (e.g., time-varying) covariates the SE can instead be over-stated.
:::
