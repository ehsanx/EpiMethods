...
### [Multiple imputation then deletion (MID)](missingdata3.html)
...
### [Testing Interactions](missingdata4i.html)

This tutorial demonstrates how to test for effect modification (interactions) in complex survey data (NHANES) while handling missing data with Multiple Imputation (MICE). The workflow involves: (1) Imputing missing data using mice to create multiple complete datasets; (2) Fitting two nested models to each imputed dataset using lapply and svyglm: a full model including the interaction term (Age * Smoking) and a reduced model with only main effects (Age + Smoking); and (3) comparing these models using the multiparameter Wald Test ($D_1$ statistic) via the D1 function. The tutorial explains how to interpret the resulting F-statistic and p-value to determine if the interaction is statistically significant and should be retained in the model.
...
### [Missing data imputation in survival analysis](missingdata12.html)
