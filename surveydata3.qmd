...
::: callout-warning
The CCHS public-use microdata file (PUMF) does not release the strata and PSU identifiers needed for design-based variance estimation; instead, Statistics Canada releases a set of **bootstrap (replicate) weights** for that purpose. Because we build the design with `id = ~1` (a weighted simple-random-sample approximation) rather than `svrepdesign(type = "bootstrap", repweights = ...)`, the **point estimates are valid**, but all standard errors, design effects (DEFF), and Rao-Scott / Thomas-Rao tests below are **illustrative weighted-SRS approximations only** and will generally **understate the true sampling variance**. For a design-correct analysis, load the bootstrap weights and use `svrepdesign()`.
:::
...
2.  These weighted-SRS DEFFs/SEs are approximations: the design-correct CCHS analysis would use the StatCan **bootstrap weights** (`svrepdesign`), and NHANES additionally requires the strata and PSU identifiers (`SDMVSTRA`, `SDMVPSU`).
