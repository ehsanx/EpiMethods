...
for(ii in 1:nrow(grid.search)) {
  ...
  # For this classification forest, ranger's prediction.error is the OOB
  # misclassification error (a proportion), not a mean squared error, so we
  # do not take its square root.
  grid.search$oob_error[ii] <- fit.rf.tune$prediction.error
}
head(grid.search)
```
