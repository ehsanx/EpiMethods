...
-   $OR_{NDE}(A=1,C=c) = \exp(\theta_1)$
-   $OR_{NIE}(A^{*}=1,C=c) = \exp(\theta_2)$

These are *natural* direct and indirect effects (the mediator is not fixed at any value; its distribution is shifted via the weights). They are not controlled effects, which would fix $M$ at a specific value such as $M=0$.
