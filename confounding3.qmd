...
D <- DAG.empty()
D <- D +
  node("A", distr = "rbern", prob = plogis(0)) +
...
Note that the bias from adjusting for the collider L does not go away as the sample size grows: collider bias is a *structural* (asymptotic) bias built into the data-generating process, not a small-sample artifact. Even with 1,000,000 observations the collider-adjusted estimate remains biased, while the unadjusted estimate recovers the true effect.
