...
::: callout-note
This chunk is illustrative provenance code and is therefore not evaluated (`eval = FALSE`). The intermediate file `xu-et-al-2020-nhanes-data.rds` is **not** shipped with this repository; it is the object produced by running the harmonization steps shown above on the multi-cycle public-use NHANES files (and linking the public-use mortality data, as in the [Linking mortality data](accessing7.html) tutorial). To reproduce it on a fresh machine, run the preceding download/harmonization steps yourself to create `d.final`, then export and reload it as shown. The chunk is included to document how the saved analytic datasets were derived, not to be run as-is.
:::
