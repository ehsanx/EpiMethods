...
-   Also, in contrast to the mixed effects models where we assume that the error term and the random effects (for subject $i$) both follow a normal distribution, GEE makes no full likelihood (distributional) assumption: it only specifies a mean model, a variance function, and a working correlation structure for the within subjects/clusters measurements. Hence, we can use GEE for non-normal data such as skewed data, binary data, or count data.
:::
...
summary(resp_lmer)$coefficients # Conditional (subject-specific) model
summary(resp_afex)$coefficients # Conditional (subject-specific) model
summary(resp_gee3)$coefficients # Marginal (population-averaged) model
summary(resp_gee4)$coefficients # Marginal (population-averaged) model
```

These two approaches estimate different parameters, so their coefficients are not expected to be numerically equal even when both fit the data well. The generalized mixed effect model is *conditional* on the random effects (a subject-specific effect), whereas the GEE is a *marginal* (population-averaged) model. For a nonlinear link such as the logit, the conditional (subject-specific) log-odds coefficients are, in theory, larger in magnitude than the corresponding marginal (population-averaged) coefficients; this attenuation of the marginal estimates is a feature of the different estimands, not evidence that either model is biased.
