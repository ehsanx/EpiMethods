...
::: callout-warning
The AUC and Archer-Lemeshow statistics below are computed **in-sample** (on the same data used to fit the model, with no `newdata`, train/test split, or cross-validation). They therefore measure **apparent** (optimistic) performance and will overstate how the model generalizes. For a more honest assessment, use a survey-aware split-sample or cross-validation / optimism-correction approach as demonstrated in the [predictive factors module](predictivefactors7.html).

Note also that pooling the per-imputation results below by averaging the AUC and taking the **majority** of Archer-Lemeshow p-values (`> 0.05`) are pragmatic heuristics. They are **not** a formal application of Rubin's rules, which are defined for parameter estimates and their variances rather than for AUC or goodness-of-fit p-values.
:::
