...
Both models are fitted with the `binomial` family and `fit3` is nested within `fit9` (the latter simply adds `age`), so their AIC values are directly comparable. The model with `age` (`fit9`) returns the lower AIC, indicating that adding age improves the design-based fit. Note that `AIC.svyglm` reports the design-based AIC of @lumley2015aic (the Lumley-Scott AIC), which adjusts the usual penalty for the complex survey design rather than using the naive number of parameters.
