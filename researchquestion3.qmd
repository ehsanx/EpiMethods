...
### Working with a causal (associational) question using CCHS
...
```{r bmi1, echo=FALSE, out.width = '95%'}
...
c123sub3$smoke <- car::recode(c123sub3$smoke,
                        "c('DAILY','OCCASIONAL',
                         ...
                         c('FORMER DAILY','FORMER OCCASION.') = 'Former smoker';
...
# Sanity check: true zeros must not be dropped to NA by the cut()
fruit.num <- as.numeric(as.character(c123sub3$fruit.cont))
stopifnot(all(!is.na(c123sub3$fruit2[!is.na(fruit.num) & fruit.num == 0])))
