...
df <- data.frame(
  Term = c("Y", "A", "W", "family", "V.Q", "V.g", "Q.SL.library", "g.SL.library"),
  Description = c(
    "Outcome vector",
    "Exposure vector",
    "Matrix that includes vectors of all covariates",
    "Distribution",
    "Cross-validation folds for outcome modeling",
    "Cross-validation folds for exposure modeling",
...
The `V.Q` and `V.g` arguments set the number of cross-validation folds for the outcome and exposure models separately; alternatively, a single `V` argument sets the same number of folds for both.
...
n_eff_exp <- min(n, 5*(n*min(p_exp, 1-p_exp)))
n_eff_exp
```

For both models, the effective sample size equals our sample size, $n = 5,735$. Note that $n_{eff}$ floors to $n$ here only because the outcome (and exposure) are common; with a rare outcome the effective sample size would be smaller than $n$ (see [the SuperLearner tutorial](machinelearningCausal2.html) for the formula).
...
::: callout-note
For a binary outcome, the `tmle` package returns three marginal estimands: the ATE on the risk-difference scale (`estimates$ATE`), the marginal risk ratio (`estimates$RR`), and the marginal odds ratio (`estimates$OR`). Below we extract `estimates$OR$psi`, so the reported quantity is the **marginal odds ratio**, not the ATE (risk difference).
:::

Marginal odds ratio for binary outcome using user-specified library: `r round(tmle.est.bin, 2)` and 95% CI is `r tmle.ci.bin`
...
Marginal odds ratio for binary outcome using default library: `r round(tmle.est.bin.def, 2)` with 95% CI `r tmle.ci.bin.def`.

The marginal odds ratio when using the default SuperLearner library is very close to the marginal odds ratio when using our user-specified SuperLearner library. However, the confidence interval from TMLE using the default SuperLearner library is slightly wider than the confidence interval from TMLE using our user-specified SuperLearner library.
...
fit0.bin <- glm(Death~RHC.use, data = ObsData, family="binomial")
require(Publish)
