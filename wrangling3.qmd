...
The `levels()<-` assignment can also be used to collapse several levels of a factor into fewer levels.
...
::: column-margin
`]` stands for closed interval, i.e., right = TRUE. On the other hand, `)` means open interval. `cut()` defaults to right-closed intervals (right = TRUE). The choice only changes the assignment of observations whose value falls exactly on a break point (e.g., a value of 14 or 18 here); all other observations keep the same bin. Even so, there can be a huge difference when setting right = TRUE vs. right = FALSE if many observations sit on the break points (as in this example).
:::
