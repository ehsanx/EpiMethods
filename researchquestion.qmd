...
In this book we draw the distinction by the role the variables play rather than by the modelling machinery: a question is treated as **causal/associational** when there is a single primary exposure whose effect on the outcome we want to isolate by adjusting for confounders (and the causal interpretation rests on the usual identification assumptions). A question is treated as **predictive** when there is no single exposure of interest and we instead use many predictors together to forecast the outcome as accurately as possible, without attaching a causal meaning to any individual coefficient.
